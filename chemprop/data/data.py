...
class MoleculeDatapoint:
    ...
    def __init__(self,
                 ...
                 temperatures: List[Optional[float]] = None,
        ...
        self.raw_temperature = self.temperature
    ...
    def set_t(self, t: List[float]):
        """
        Sets the temperature of a datapoint.

        :param ts: A list of floats containing the temperature.
        """
        self.temperature = t
    ...
    def reset_features_and_targets(self) -> None:
        ...
        self.atom_descriptors, self.atom_features, self.bond_features = \
            self.raw_atom_descriptors, self.raw_atom_features, self.raw_bond_features
        self.temperature = self.raw_temperature
...
class MoleculeDataset(Dataset):
    ...
    def temperatures(self) -> List[float]:
        """
        Returns the temparatures associated with the molecules.

        :return: A list of temparature.
        """
        return [d.temperature for d in self._data]
        ...
        if scale_atom_descriptors and not self._data[0].atom_descriptors is None:
            for d in self._data:
                d.set_atom_descriptors(
                    self._scaler.transform(d.raw_atom_descriptors))
        elif scale_atom_descriptors and not self._data[0].atom_features is None:
            for d in self._data:
                d.set_atom_features(
                    self._scaler.transform(d.raw_atom_features))
        elif scale_bond_features:
            for d in self._data:
                d.set_bond_features(
                    self._scaler.transform(d.raw_bond_features))
        else:
            for d in self._data:
    ...
    def set_ts(self, ts: List[List[float]]):
        """
        Sets the temperature for each molecule in the dataset. Assumes the temperatures are aligned with the datapoints.

        :param ts: A list of lists of floats (or None) containing temperature for each molecule. This must be the
                        same length as the underlying dataset.
        """
        assert len(self._data) == len(ts)
        for i in range(len(self._data)):
            self._data[i].set_t(ts[i])
...
def make_mols(smiles: List[str], reaction: bool, keep_h: bool):
    ...
    """
    if reaction:
        mol = [SMILES_TO_MOL[s] if s in SMILES_TO_MOL else (make_mol(
            s.split(">")[0], keep_h), make_mol(s.split(">")[-1], keep_h)) for s in smiles]
    else:
        mol = [SMILES_TO_MOL[s] if s in SMILES_TO_MOL else make_mol(
            s, keep_h) for s in smiles]
    return mol
