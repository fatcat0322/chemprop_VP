...
def get_task_names(path: str,
                   ...
                   temperature_columns: List[str] = None) -> List[str]:
    """
    ...
    ignore_columns = set(
        smiles_columns + ([] if ignore_columns is None else ignore_columns) + ([] if temperature_columns is None else temperature_columns))
...
def get_data_weights(path: str) -> List[float]:
    ...
    if min(weights) < 0:
        raise ValueError(
            'Data weights must be non-negative for each datapoint.')
    return weights
...
def get_data(path: str,
             ...
             target_columns: List[str] = None,
             ignore_columns: List[str] = None,
             temperature_columns: List[str] = None,
    ...
    if args is not None:
        ...
        temperature_columns = temperature_columns if temperature_columns is not None else args.temperature_columns
    ...
    if phase_features_path is not None:
        ...
        if features_data is not None:
            features_data = np.concatenate(
                (features_data, phase_features), axis=1)
    ...
    with open(path) as f:
        ...
        all_smiles, all_targets, all_rows, all_features, all_phase_features, all_weights, all_temperatures = [
        ], [], [], [], [], [], []
        for i, row in enumerate(tqdm(reader)):
            smiles = [row[c] for c in smiles_columns]

            targets = [float(row[column]) if row[column] not in [
                '', 'nan'] else None for column in target_columns]

            temperatures = [float(row[column]) if row[column] not in [
                '', 'nan'] else None for column in temperature_columns]
        ...
        data = MoleculeDataset([
            ...
            ) for i, (smiles, targets, temperatures) in tqdm(enumerate(zip(all_smiles, all_targets, all_temperatures)),
...
def split_data(data: MoleculeDataset,
    ...
    # Use to constrain data with the same smiles go in the same split. Considers first molecule only.
